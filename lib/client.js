...
(function() {
  ...
  Monarch.Util.Inflection = (function() {
    var isHash, rules;
    rules = {
  ...
  Monarch.Util.Visitor = (function() {
    var visitPrimitive, visiteeName;
  ...
  Monarch.Field = (function() {
    ...
    Field.prototype.wireRepresentation = function() {
      return Monarch.Json.serialize(this.getValue());
    };
  ...
  Monarch.Relations.Relation = (function(_super) {
    ...
    Relation.fromJson = function() {
      var args, _ref;
      args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return (_ref = Monarch.Json).parse.apply(_ref, args);
    };

    Relation.prototype.wireRepresentation = function() {
      return Monarch.Json.serialize(this);
    };
  ...
  Monarch.Relations.OrderBy = (function(_super) {
    ...
    OrderBy.delegate('getColumn', 'inferJoinColumns', 'columns', 'create', 'created', 'repository', {
      to: 'operand'
    });
  ...
  Monarch.Relations.Table = (function(_super) {
    ...
    Table.deriveEquality('name', 'columnsByName');
  ...
  (function() {
    var moduleName, _i, _len, _ref, _results;
    _ref = ["Expressions", "Relations"];
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      moduleName = _ref[_i];
      _results.push(_.each(Monarch[moduleName], function(klass, klassName) {
        var methodName;
  ...
  Monarch.Json = (function() {
    var Expressions, Relations, Util, camelize, capitalize, normalizeTable, normalizeTables, replaceSeparator, singularize, uncapitalize, underscore, underscoreAndPluralize, wrapScalars, _ref,
      _this = this;
    Relations = Monarch.Relations, Expressions = Monarch.Expressions, Util = Monarch.Util;
    _ref = Util.Inflection, singularize = _ref.singularize, capitalize = _ref.capitalize, camelize = _ref.camelize, underscoreAndPluralize = _ref.underscoreAndPluralize, underscore = _ref.underscore, uncapitalize = _ref.uncapitalize;
    this.serialize = Monarch.Util.Visitor.visit;
    this.parse = function(json, tables) {
      var method, type;
      type = json.type;
      method = this["parse_" + type];
      if (!method) {
        throw "Can't parse " + type + " from JSON";
      }
      return method.call(this, json, normalizeTables(tables));
    };
    this.tableNameFromJson = function(name) {
      return singularize(capitalize(camelize(name)));
    };
    this.tableNameToJson = function(name) {
      return replaceSeparator(underscoreAndPluralize(uncapitalize(name)));
    };
    this.columnNameFromJson = camelize;
    this.columnNameToJson = function(name) {
      return replaceSeparator(underscore(name));
    };
    this.visit_Relations_Table = function(r) {
      return {
        type: 'Table',
        name: this.tableNameToJson(r.name)
      };
    };
    this.visit_Relations_Projection = function(r) {
      return {
        type: 'Projection',
        operand: this.serialize(r.operand),
        table: r.table.name
      };
    };
    this.visit_Relations_Selection = function(r) {
      return {
        type: 'Selection',
        operand: this.serialize(r.operand),
        predicate: this.serialize(r.predicate)
      };
    };
    this.visit_Relations_InnerJoin = function(r) {
      return {
        type: 'InnerJoin',
        leftOperand: this.serialize(r.left),
        rightOperand: this.serialize(r.right),
        predicate: this.serialize(r.predicate)
      };
    };
    this.visit_Relations_Limit = function(r) {
      return {
        type: 'Limit',
        operand: this.serialize(r.operand),
        count: r.count
    ...
    this.visit_Relations_Offset = function(r) {
      return {
        type: 'Offset',
        operand: this.serialize(r.operand),
        count: r.count
    ...
    this.visit_Relations_Difference = function(r) {
      return {
        type: 'Difference',
        leftOperand: this.serialize(r.left),
        rightOperand: this.serialize(r.right)
    ...
    this.visit_Relations_Union = function(r) {
      return {
        type: 'Union',
        leftOperand: this.serialize(r.left),
        rightOperand: this.serialize(r.right)
    ...
    this.visit_Relations_OrderBy = function(r) {
      return this.serialize(r.operand);
    };
    this.visit_Expressions_Column = function(e) {
      return {
        type: 'Column',
        table: this.tableNameToJson(e.table.name),
        name: this.columnNameToJson(e.name)
      };
    };
    this.parse_Table = function(json, tables) {
      return tables[this.tableNameFromJson(json.name)];
    };
    this.parse_Projection = function(json, tables) {
      return new Relations.Projection(this.parse(json.operand, tables), tables[json.table]);
    };
    this.parse_Selection = function(json, tables) {
      return new Relations.Selection(this.parse(json.operand, tables), this.parse(json.predicate, tables));
    };
    this.parse_InnerJoin = function(json, tables) {
      return new Relations.InnerJoin(this.parse(json.leftOperand, tables), this.parse(json.rightOperand, tables), this.parse(json.predicate, tables));
    };
    this.parse_Limit = function(json, tables) {
      return new Relations.Limit(this.parse(json.operand, tables), json.count);
    };
    this.parse_Offset = function(json, tables) {
      return new Relations.Offset(this.parse(json.operand, tables), json.count);
    };
    this.parse_Difference = function(json, tables) {
      return new Relations.Difference(this.parse(json.leftOperand, tables), this.parse(json.rightOperand, tables));
    };
    this.parse_Union = function(json, tables) {
      return new Relations.Union(this.parse(json.leftOperand, tables), this.parse(json.rightOperand, tables));
    };
    this.parse_Column = function(json, tables) {
      var table;
      table = tables[this.tableNameFromJson(json.table)];
      return table.getColumn(this.columnNameFromJson(json.name));
    };
    _.each(['Equal', 'LessThan', 'GreaterThan', 'GreaterThanOrEqual', 'LessThanOrEqual', 'And', 'Or'], function(operator) {
      _this["visit_Expressions_" + operator] = function(e) {
        return {
          type: operator,
          leftOperand: wrapScalars(this.serialize(e.left)),
          rightOperand: wrapScalars(this.serialize(e.right))
        };
      };
      return _this["parse_" + operator] = function(json, c) {
        return new Expressions[operator](this.parse(json.leftOperand, c), this.parse(json.rightOperand, c));
      };
    });
    this.visit_Boolean = _.identity;
    this.visit_Number = _.identity;
    this.visit_String = _.identity;
    this.visit_null = _.identity;
    this.visit_Date = function(d) {
      return d.getTime();
    };
    this.parse_Scalar = function(json) {
      return json.value;
    };
    replaceSeparator = function(name) {
      return name.replace(/_/g, Monarch.resourceUrlSeparator);
    };
    wrapScalars = function(value) {
      if (_.isObject(value)) {
        return value;
      }
      return {
    ...
    normalizeTables = function(tables) {
      var name, result, table;
      result = {};
      for (name in tables) {
        table = tables[name];
        result[name] = normalizeTable(table);
      }
      return result;
    };
    normalizeTable = function(tableOrClass) {
      if (tableOrClass instanceof Relations.Relation) {
        return tableOrClass;
      } else {
        return tableOrClass.table;
      }
    };
    return this;
  }).call({});
  ...
  Monarch.RecordRetriever = (function() {
    return {
